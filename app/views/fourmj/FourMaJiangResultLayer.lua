...
local FourMaJiangResultLayer = class("FourMaJiangResultLayer", cc.load("mvc").DialogBase)
local helper   = require("app.views.fourmj.helper")
local DDMaJiang = require("app.views.fourmj.DDMaJiang")
FourMaJiangResultLayer.RESOURCE_FILENAME = "game_fourmj/fourmj_result_layer.csb"

function FourMaJiangResultLayer:onCreate(data) 
    print("FourMaJiangResultLayer:onCreate(data).............", data)
...
function FourMaJiangResultLayer:getHuString( dwChiHuRight )
    -- body
    local str_hu = ""
    ...
    print("FourMaJiangResultLayer:getHuString( dwChiHuRight )", dwChiHuRight, str_hu)
    return str_hu
end

--显示玩家结果
function FourMaJiangResultLayer:showPlayerResult(nIdx, nChairID, data)
    -- body
    ...
    --检测杠
    local nQnGangCount = 0
    local nMingGangCount = 0
    local nDianGangCount = 0
    for k,v in pairs(data.state_list[nChairID]) do
        if v[1].type == 2 then
            nQnGangCount = nQnGangCount + 1 
        elseif v[1].type == 4 then
            nDianGangCount =  nDianGangCount + 1
        elseif v[1].type == 5 then
            nMingGangCount =  nMingGangCount + 1
        end
    end
    if nQnGangCount > 0 then
        temp = temp .. "暗杠X" .. nQnGangCount .. " "
    end
    if nMingGangCount > 0 then
        temp = temp .. "巴杠X" .. nMingGangCount .. " "
    end
    if nDianGangCount > 0 then
        temp = temp .. "点杠X" .. nDianGangCount .. " "
    end
    ...
    for k,v in pairs(data.state_list[nChairID]) do
        if v[1].type == 1 then
            for i=1, 3 do
                local p_mj = DDMaJiang.new(4,1, v[i].id)
                p_mj:setScale(0.7)
    ...
    for k,v in pairs(data.state_list[nChairID]) do
        if v[1].type == 2 then
            ...
            for i=1,4 do
                local p_mj
                if v[i].id == 10 then
                    p_mj = DDMaJiang.new(4, 1, v[i].id)
                else
                    if i == 1 and nYaoJiCount > 0 then
                        p_mj = DDMaJiang.new(4, 1, v[i].id)
                    else
                        p_mj = DDMaJiang.new(4, 2, v[i].id)
                    end
                    if i == 2 then
                        p_mj = DDMaJiang.new(4, 2, v[i].id)
                        centerCard = p_mj
                ...
                p_mj:setScale(0.7)    
                if i == 4 and centerCard ~= nil then
                    p_mj:setPosition(centerCard:getContentSize().width/2, centerCard:getContentSize().height/2+15)
                    centerCard:addChild(p_mj,10)
                else
                    p_mj:setPosition((n-1)*p_mj:getContentSize().width*0.7 - x, s.height/2)
                    cardPanel:addChild(p_mj)
                    n = n + 1
...
function FourMaJiangResultLayer:showButtonEffect()
    -- body
